...
import metadraftreports as mrpt
import cbmpy
...
try:
    ...
    from PyQt4.QtGui import (QCursor, QBrush, QColor, QPalette, QPixmap)
...
try:
    ...
    from PyQt5.QtGui import (QCursor, QBrush, QColor, QPalette, QPixmap)
...
class MetaDraftGUI(QWidget):
    ...
    def func_formatSummaryReport(self):
        ...
        html += '<tr><td><strong>{}</strong></td><td><strong>{}</strong></td><td><strong>{}</strong></td><td><strong>{}</strong></td></tr>'.format('Model', 'Per. Total', 'Num. Genes', 'Avg. score')
        for o in gene_tbl:
            html += '<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(o[1], round(o[0]/total_sel*100.0, 2), o[0],\
                                                                        round(gene_stats['score'][o[1]]/gene_stats['org'][o[1]], 3))
        html += """
            </tbody>
        </table>
        <br/>
        ...
        """
        cp = '<p><small>Generated by MetaDraft {} &copy; Brett G.Olivier, Vrije Universiteit Amsterdam, 2017</small></p>'.format(__version__)
    ...
    def buildSBMLModel(self):
        ...
        for r_ in selected_reactions:
            ...
            used_gids = []
            if len(gids) == 1:
                if gids[0] not in self._gene_selected_ids_:
                    gene_ass = gene_ass.replace(gids[0], 'unknown')
                else:
                    used_gids.append(gids[0])
            else:
                for g_ in gids:
            ...
            for g_ in used_gids:
                if tlx.count(g_) > 1:
                    print(g_, tlx.count(g_))
                else:
                    gene_ass = gene_ass.replace(g_, slx[tlx.index(g_)])

            if not DUP_R:
                self.model.createGeneProteinAssociation(ridnew, gene_ass)
                R.setAnnotation('GENE_ASSOCIATION', gene_ass)
                ...
                if self.NO_EXPORT_SEQ:
                    for a_ in list(R.annotation):
                        if a_.startswith('gbank_seq_'):
                            R.annotation.pop(a_)
            #else:
                #ga_old = self.model.getReaction(rid).getAnnotation('GENE_ASSOCIATION')
                #gao_old = self.model.getReaction(rid).getAnnotation('GENE_ASSOCIATION_ORIGINAL')
                #ga_new = '({}) or ({})'.format(ga_old, gene_ass)
                #gao_new = '({}) or ({})'.format(gao_old, gene_ass_original)
                #g_ = self.model.gpr.pop(self.model.gpr.index(self.model.getGPRforReaction(rid)))
                #del g_
                #self.model.createGeneProteinAssociation(rid, ga_new)
                #R.setAnnotation('GENE_ASSOCIATION', ga_new)

        for g_ in self.model.getGeneIds():
    ...
    def bp_loadResult(self):
        ...
        #self.appwindow.setWindowTitle('MetaDraft [{}]'.format(os.path.split(self.result_file)[-1].replace('_metalink.resplus.json','')))
